...
from app.modules.attractions.schemas.attraction import AttractionUpdate, AttractionProcessUpdate, AttractionCreateDraft
...
async def update_attraction_draft(
    attraction_id: str,
    data: AttractionCreateDraft,
) -> Dict[str, Any]:
    """Update a draft attraction's minimal fields."""
    ...
    try:
        ...
        if data.attraction_name:
            update_payload["name"] = data.attraction_name
        if data.latitude is not None:
            update_payload["latitude"] = data.latitude
        if data.longitude is not None:
            update_payload["longitude"] = data.longitude
        ...
        updated = await prisma.attraction.update(
            where={"id": attraction_id},
            data=update_payload,
            include={
                "gallery": True,
        ...
        logger.exception("update_attraction_draft error")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e))
