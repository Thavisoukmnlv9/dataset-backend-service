"""List bars with filters and pagination (no vector search)."""
import logging
...
def _serialize_bar_for_list(r: Any) -> Dict[str, Any]:
    if not r:
        return {}
    return {
        "id": r.id,
...
def _build_where(filters: BarFilters) -> Dict[str, Any]:
    where: Dict[str, Any] = {}
    if filters.province:
...
async def get_bars(
    ...
    """List bars with optional text search (name, description, slug)."""
    try:
        ...
        items = await prisma.bar.find_many(
            where=where,
            order=order,
        ...
        logger.exception("get_bars error")
